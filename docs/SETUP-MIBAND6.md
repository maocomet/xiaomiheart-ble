...
- **在本项目当前这套环境（这台 Mi Band 6 + 当前版本的 Gadgetbridge）中，开启后出现过手环重连异常。**

> ⚠️ 上一条是本环境下的**实测观察**，**不代表所有设备、所有 Gadgetbridge 版本都会如此**，也不构成对 BLE Intent API 的一般性结论。只是对本项目而言：没有理由开它，而开它有已知风险，所以保持关闭。
